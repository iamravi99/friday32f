...
const ViralManager = () => {
  ...
  const handleUpdate = async () => {
    try {
      await api.put(`/viral/${editItem._id}`, editItem);
      setEditItem(null);
      fetchData();
  ...
  return (
    <div className="bg-zinc-950 text-white min-h-screen">
      <Adminnav />
      <div className="p-8">
        <h2 className="text-2xl font-bold text-pink-500 mb-4">⚡ Manage Viral Content</h2>

      {/* Add Form */}
      <div className="bg-zinc-900 p-4 rounded mb-6">
        <h3 className="text-lg font-bold mb-3">Add New Viral Content</h3>
        ...
        />
        <button onClick={handleAdd} className="bg-pink-600 px-4 py-2 rounded mt-3">Add Viral Content</button>
      </div>
      ...
      {editItem && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-zinc-900 p-6 rounded-lg w-[800px] max-h-[80vh] overflow-y-auto">
            <h3 className="text-lg font-bold mb-3">Edit Viral Content</h3>
